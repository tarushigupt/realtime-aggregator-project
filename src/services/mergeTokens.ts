...
function chooseNumericPreferHigherLiquidity(existing: TokenData, incoming: TokenData, field: keyof TokenData): number | undefined {
  const eVal = existing[field] as any;
  const iVal = incoming[field] as any;

  if (eVal == null && iVal == null) return undefined;
  if (eVal == null) return iVal;
  if (iVal == null) return eVal;
...
export function mergeTokenLists(lists: TokenData[][]): TokenData[] {
  const map = new Map<string, TokenData>();

  for (const list of lists) {
    for (const raw of list) {
      const incoming = normalizeToken(raw);
      ...
      let volume_sol: number | undefined;
